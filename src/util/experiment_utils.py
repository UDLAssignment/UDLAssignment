...
def run_point_estimate_initialisation(model, data, epochs, task_ids, batch_size,
    ...
    loader = DataLoader(task_data, batch_size, num_workers=20)
...
def eval_model(model, model_cs_trained, test_data, y_transform, task_idx, \
    device,  multiheaded, test_task_ids, train_full_coreset: bool = True, \
        coreset=None, optimizer=None, epochs=None):
...
def run_task(model, train_data, train_task_ids, test_data, test_task_ids,
             task_idx, coreset, config, device, lr, y_transform=None, summary_writer=None):

    head = task_idx if config.multiheaded else 0
    ...
    train_loader = DataLoader(non_coreset_data, config.batch_size)
    ...
    for epoch in tqdm(range(config.epochs), 'Epochs: '):
        ...
        for batch in train_loader:
            ...
            loss_kl,loss_logprob = model.vcl_loss(x, y_true, head, len(task_data))
            
            loss = loss_kl + loss_logprob

            summary_writer.log({
                f"loss/kl": loss_kl,
                f"loss/logprob": loss_logprob,
                f"loss/total": loss
            })
    ...
    save_model(optimizer, config.name + "/precs_optim_" + str(task_idx) + ".pt")
    ...
    if config.train_full_coreset:
        model_cs_trained = coreset.coreset_train(
            model, optimizer, list(range(task_idx + 1)), config.epochs,
            device, y_transform=y_transform, multiheaded=config.multiheaded)
    else:
        model_cs_trained = False

    # test
    task_accuracies, mean_accuracy = eval_model(model, model_cs_trained, test_data, y_transform, task_idx, \
        device,  config.multiheaded, test_task_ids, config.train_full_coreset, \
        coreset, optimizer, config.epochs)
        
    if summary_writer is not None:
        task_accuracies_dict = dict(zip(["TASK_" + str(i) for i in range(task_idx + 1)], task_accuracies))
        summary_writer.log({
            **{f"accs/" + k: v for k,v in task_accuracies_dict.items()},
            f"accs/mean_accuracy": mean_accuracy
        })
   
    write_as_json(config.name + f'/accuracy_{task_idx}.txt', task_accuracies)
    save_model(model, config.name + '/model_task_' + str(task_idx) + '.pth')
    save_model(model_cs_trained, config.name + '/model_task_' + str(task_idx) + 'post_corset.pth')
    save_model(coreset, config.name + "/coreset_" + str(task_idx) + ".pt")
