...
class Coreset():
    ...
    def coreset_train(self, m, old_optimizer, tasks, epochs, device,
        ...
        try:
            optimizer.load_state_dict(old_optimizer.state_dict(), strict=False)
        except:
        ...
        for _ in tqdm(range(5), 'Epochs: '):
            ...
            for task_idx in tasks:
                ...
                for batch in train_loaders[task_idx]:
                    ...
                    kl_loss,logprob_loss = model.vcl_loss(x, y_true, head, len(self.coreset))
                    loss = kl_loss + logprob_loss
