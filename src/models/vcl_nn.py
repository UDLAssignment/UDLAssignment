...
class DiscriminativeVCL(nn.Module):
    ...
    def __init__(self, config):
        super().__init__()
        self.input_size = config.mnist_flattened_dim
        self.out_size = config.n_classes
        ...
        print("Number of heads:", self.n_heads)

        self.prior, self.posterior = None, None
        self.head_prior, self.head_posterior = None, None

        self._init_variables(config.initial_posterior_var)
    ...
    def forward(self, x, head, sample_parameters=True):
        ...
        sampled_layers = self._sample_parameters(w_means, b_means, w_log_vars, b_log_vars, sample_parameters=sample_parameters)
        sampled_head_layers = self._sample_parameters(head_w_means, head_b_means, head_w_log_vars, head_b_log_vars, sample_parameters=sample_parameters)
    ...
    def vcl_loss(self, x, y, head, task_size, num_samples=TRAIN_NUM_SAMPLES) -> torch.Tensor:
        ...
        """
        # print( self.posterior[0][0][0].sum())
        # breakpoint()
        # return self._calculate_kl_term(head) / task_size - self._log_prob(x, y, head, num_samples=1, sample_parameters=False)
        return self._calculate_kl_term(head) / task_size - self._log_prob(x, y, head, num_samples=TRAIN_NUM_SAMPLES, sample_parameters=True)
    ...
    def point_estimate_loss(self, x, y, head_idx=0):
        ...
        x = x @ head_w_means[head_idx] + head_b_means[head_idx]
    ...
    def prediction(self, x, head, num_samples=TEST_NUM_SAMPLES):
        ...
        outputs = torch.empty(num_samples, len(x), self.out_size, device=x.device)
    ...
    def _log_prob(self, x, y, head, num_samples, sample_parameters=True):
        outputs = []
        for i in range(num_samples):
            outputs.append(self.forward(x, head, sample_parameters=sample_parameters))

        return - nn.CrossEntropyLoss()(torch.cat(outputs), y.repeat(num_samples).view(-1))

    def _sample_parameters(self, w_means, b_means, w_log_vars, b_log_vars, sample_parameters=True):
        ...
        for layer_n in range(len(w_means)):
            ...
            if sample_parameters:
                ...
                sampled_weights.append(w_means[layer_n])
                sampled_bias.append(b_means[layer_n])
